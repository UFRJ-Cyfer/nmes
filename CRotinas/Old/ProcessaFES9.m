function [R,SF]=ProcessaFES9(Sinal,CR,prot)
global R
...
for ciclo=1:size(R.Trechos,1) %numero de ciclos
    ...
    %disp('EAM')
    %mean(abs(Sinal.Dado(ind,indErro)))
    R.TT.ErroAbsMedio(ciclo)=mean(abs(Sinal.Dado(ind,indErro)));
    %disp('EMA')
    %abs(mean(Sinal.Dado(ind,indErro)))
...
if CR.grafCiclos==1;
    ...
    figure('Color',[1 1 1],'name',Sinal.File(1:end-4),'NumberTitle','off');
    plot(tempo,Sinal.Dado(:,R.indCon));hold on;
    plot(tempo,Sinal.Dado(:,indRef),'r');
