function NMES_GrafTempoSeparadoPB4(Sinal,R,CR,crg,cond)   
tamFonte=10;
...
if crg.correntesat==1 %marcação de corrente saturada
    ...
    if crg.Ingles~=1
        if~isempty(indsat6)
            LB={sprintf('Zona Saturada Biceps')};
        end
        if~isempty(indsat7)
            LT={sprintf('Zona Saturada Triceps')};
        end
    else
        if~isempty(indsat6)
    ...
    if~isempty(indsat6)
        ...
        for p=1:length(ii)-1
            ...
            pH(length(pH)+1)=plot(tempo(indsat6(temp)),Goni(indsat6(temp)),...
    ...
    if~isempty(indsat7)
        ...
        for p=1:length(ii)-1
            ...
            plot(tempo(indsat7(temp)),Goni(indsat7(temp)),'Color',[1 1 1],'linewidth',largLin+1,'LineStyle','-')
                        pH(length(pH)+1)=plot(tempo(indsat7(temp)),Goni(indsat7(temp)),...
...
legend(pH,leg,'Location','NorthEastOutside','FontSize',tamFonte,'FontName',nomFonte)
legend('boxoff')
